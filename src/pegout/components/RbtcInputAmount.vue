...
export default {
  ...
  setup() {
    ...
    const selectedFee = computed<MiningSpeedFee>(() => pegOutTxState.value.selectedFee as MiningSpeedFee);
    let isMaxValueZero = false;
    const amountErrorMessage = computed(() => {
      ...
      if (rbtcAmount.toString() === '0' && isMaxValueZero) {
    ...
    const safeAmount = computed((): WeiBig => {
      return new WeiBig(rbtcAmount.value ?? '0', 'rbtc');
    });
    ...
    const isWalletConnected = computed((): boolean => {
      return web3SessionState.value.account !== undefined;
    });
