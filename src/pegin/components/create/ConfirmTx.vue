    ...
    <v-row>
      <v-col v-for="step, idx in walletService.confirmationSteps()" :key="`step-${idx}`">
        ...
        <v-row class="mx-0 my-3 d-flex justify-center">
          ...
          <div class="number">{{ idx + 1 }}</div>{{ step.title }}</h4>
        </v-row>
          ...
          <v-row class="ma-0 text-center">
            <v-col>
              ...
              <v-row v-if="step.outputsToshow.opReturn.value"
                justify="center" class="mt-5 pa-0 d-none d-lg-block">
              ...
              <v-row v-if="step.outputsToshow.opReturn.value"
                  justify="center" class="ma-0 d-lg-none">
                <v-col class="mt-5 pa-0 d-flex flex-column align-center">
...
import { useState, useGetter } from '@/common/store/helper';
import {
