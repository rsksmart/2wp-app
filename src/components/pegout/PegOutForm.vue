    ...
    <v-col class="px-0">
      <v-row class="mx-0">
        <v-col cols="1" class="pa-0 d-flex align-center">
          <v-img position="center left" src="@/assets/exchange/arrow.png" height="40" contain />
        </v-col>
        <v-col class="px-0">
      ...
      <v-row class="exchange-form">
        <v-col cols="8" lg="8">
          <rsk-wallet-connection @switchDeriveButton="switchDeriveButton" />
...
=======
                  <v-container v-if="injectedProvider === appConstants.RLOGIN_LIQUALITY_WALLET">
                    <div>
                      ...
                      to get the destination address.
>>>>>>> 45c295905635bb04ca0a39e0d135ac4cc4cb7bf1
                    </div>
                  </v-container>
                </v-row>
              </v-col>
        ...
        <v-col cols="4" lg="4">
          <tx-summary-fixed :summary="pegOutFormSummary" :initialExpand="true" :type="typeSummary"
      ...
      <v-row class="mx-0">
        <v-col cols="2" class="d-flex justify-start ma-0 pa-0">
          <v-btn @click="back" rounded outlined color="#000000" width="110"
            :disabled="pegOutFormState.matches(['loading', 'goingHome'])">
            <span>Back</span>
          </v-btn>
        </v-col>
        <v-col cols="10" class="d-flex justify-end ma-0 py-0 pl-0">
          <v-btn v-if="!pegOutFormState.matches(['loading'])" rounded color="#000000" @click="send"
    ...
    <template v-if="showTxErrorDialog">
      <tx-error-dialog :showTxErrorDialog="showTxErrorDialog" :errorMessage="txError"
